...
- 买入后下一根 K 线正式收盘低于压力带下沿：按该根收盘市值止损
- 支持买入后首根 1 分钟 K 线收盘跌破压力带下沿止损、动态锁盈、止盈区间扫描、固定手续费
- 买卖点会在前端 K 线上标记 `B/S`
...
- `gmgn.api_key` / `gmgn.api_keys` / `gmgn.max_qps`：配置中的 Key 启动时写入 PostgreSQL `gmgn_api_keys`；所有 GMGN K 线、报价、项目发现和 symbol 查询统一轮询数据库中的 `available` Key，并共享全局 `10 QPS` 限速，收到 `429` 的 Key 冷却 `60s`
...
- `signal.poll_interval_seconds` / `signal.min_market_cap`：候选池监控轮询间隔与未买入低市值移除阈值；候选池移除阈值最低为 `15_000`，配置仅可提高该阈值
- XXYY/GMGN 自动发现的项目只有在上游当前市值严格大于 `15_000` 时才允许首次进入候选池；已入池且未买入的项目在实时市值低于 `15_000` 时直接移除
...
- Redis 实时持仓会在服务启动时合并回运行时持仓，避免数据库异步落库暂时失败后重启丢失可卖仓位；候选池查不到买入信号但确认 Redis 存在未平仓仓位时，按实际成交价恢复卖出监控
- 交易异步落库任务失败后会延迟重试，但不会阻塞队列中的后续信号、订单和成交任务
