<template>
  <main class="workspace">
    <header class="hero compact-hero">
      <el-button type="primary" size="large" :loading="store.loading" @click="loadKlineLevels">加载 K 线并计算</el-button>
      <el-button size="large" :disabled="!selectedChartRange" :loading="store.loading" @click="loadSelectedRangeLevels">计算选中区域压力位</el-button>
      <el-button size="large" :disabled="!selectedChartRange && !loadedRangeLabel" @click="clearSelectedRange">恢复近 5 天</el-button>
      <el-button type="success" size="large" :loading="store.backtestLoading" @click="runStrategyForLoadedRange">执行回测</el-button>
    </header>
    ...
    <section class="query-panel">
      <div class="query-overview">
        <div class="query-range-card">
          ...
          <div class="query-range-meta">{{ loadedRangeLabel || '默认使用当前时刻向前近 5 天' }}</div>
        </div>
        ...
        <section class="query-group">
          ...
          <div class="query-group-grid">
            <label class="query-field">
              ...
              <el-input-number v-model="form.windowSize" class="query-number" :min="20" :step="20" />
            </label>
            <label class="query-field">
              <span class="query-label">压力带窗口K线数</span>
              <el-input-number v-model="form.levelWindowSize" class="query-number" :min="20" :step="20" />
            </label>
            <label class="query-field">
              <span class="query-label">压力带窗口步长</span>
              <el-input-number v-model="form.levelWindowStep" class="query-number" :min="1" :step="1" />
            </label>
            <label class="query-field">
              <span class="query-label">带宽范围(%)</span>
              <el-input-number v-model="form.bandRangePercent" class="query-number" :min="0.1" :step="0.1" :precision="1" />
            </label>
          </div>
        </section>
        ...
        <section class="query-group">
          ...
          <div class="query-group-grid query-group-grid-compact">
            <label class="query-field">
              ...
              <el-input-number v-model="form.minTouches" class="query-number" :min="2" :step="1" />
            </label>
            <label class="query-field">
              <span class="query-label">突破确认根数</span>
              <el-input-number v-model="form.confirmBars" class="query-number" :min="1" :step="1" />
            </label>
          </div>
        </section>
            ...
            <div class="strategy-topline">
              <label class="query-field strategy-method-select">
                ...
                <el-select v-model="strategyForm.methodCode">
                  <el-option v-for="method in strategyMethodOptions" :key="method.code" :label="method.name" :value="method.code" />
                </el-select>
              </label>
              <div class="strategy-overview-chips">
                <div class="strategy-overview-chip">
                  <span>止盈扫描</span>
                  <strong>{{ strategyForm.takeProfitRateStartPercent }}% → {{ strategyForm.takeProfitRateEndPercent }}%</strong>
                  <em>每 {{ strategyForm.takeProfitRateStepPercent }}% 一组</em>
                </div>
                <div class="strategy-overview-chip">
                  <span>成本与仓位</span>
                  <strong>{{ strategyForm.feeRatePercent }}% 费率 · {{ strategyForm.positionSizeUsd }}u</strong>
                  <em>硬止损 -{{ strategyForm.hardStopLossRatePercent }}% · 触发 {{ strategyForm.activationProfitRatePercent }}% / 锁盈 {{ strategyForm.lockedProfitRatePercent }}%</em>
                </div>
              </div>
            </div>
              ...
              <section class="strategy-field-section">
                ...
                <div class="strategy-field-grid">
                  <label class="query-field">
                    ...
                    <el-input-number v-model="strategyForm.takeProfitRateStartPercent" class="query-number" :min="1" :max="50" :step="1" />
                  </label>
                  <label class="query-field">
                    <span class="query-label">止盈终点(%)</span>
                    <el-input-number v-model="strategyForm.takeProfitRateEndPercent" class="query-number" :min="1" :max="50" :step="1" />
                  </label>
                  <label class="query-field">
                    <span class="query-label">止盈步长(%)</span>
                    <el-input-number v-model="strategyForm.takeProfitRateStepPercent" class="query-number" :min="0.5" :max="10" :step="0.5" :precision="1" />
                  </label>
                </div>
              </section>
              <section class="strategy-field-section">
                ...
                <div class="strategy-field-grid strategy-field-grid-risk">
                  <label class="query-field">
                    ...
                    <el-input-number v-model="strategyForm.feeRatePercent" class="query-number" :min="0" :max="30" :step="0.1" :precision="1" />
                  </label>
                  <label class="query-field">
                    <span class="query-label">单笔投入(U)</span>
                    <el-input-number v-model="strategyForm.positionSizeUsd" class="query-number" :min="1" :step="1" />
                  </label>
                  <label class="query-field">
                    <span class="query-label">硬止损(%)</span>
                    <el-input-number v-model="strategyForm.hardStopLossRatePercent" class="query-number" :min="0.1" :max="50" :step="0.5" :precision="1" />
                  </label>
                  <label class="query-field">
                    <span class="query-label">动态止损触发(%)</span>
                    <el-input-number v-model="strategyForm.activationProfitRatePercent" class="query-number" :min="1" :max="50" :step="1" />
                  </label>
                  <label class="query-field">
                    <span class="query-label">动态锁盈(%)</span>
                    <el-input-number v-model="strategyForm.lockedProfitRatePercent" class="query-number" :min="1" :max="50" :step="1" />
                  </label>
                </div>
              </section>
            </div>
    ...
    <section class="status-strip">
      <div class="status-item"><span>Source</span><strong>Birdeye K 线</strong></div>
      <div class="status-item"><span>Interval</span><strong>{{ form.interval }}</strong></div>
      <div class="status-item"><span>Bars</span><strong>{{ barCount }}</strong></div>
      <div class="status-item"><span>Windows</span><strong>{{ windowCount }}</strong></div>
      <div class="status-item"><span>Support</span><strong>{{ supportCount }}</strong></div>
      <div class="status-item"><span>Resistance</span><strong>{{ resistanceCount }}</strong></div>
      <div class="status-item"><span>Window</span><strong>{{ activeWindow }}</strong></div>
    </section>

    <section v-if="selectedChartRange" class="panel selection-panel">
      <div>
        <strong>已选中 K 线区域</strong>
        <span>{{ formatShortTime(selectedChartRange.startTime) }} - {{ formatShortTime(selectedChartRange.endTime) }} · {{ selectedChartRange.klineCount }} 根 K 线</span>
      </div>
      <div class="selection-actions">
        <el-button type="primary" :loading="store.loading" @click="loadSelectedRangeLevels">计算该区域</el-button>
    ...
    <section v-if="windowOptions.length" class="panel">
      <div class="panel-heading">
        <div>
          ...
          <div class="panel-subtitle">仅保留 n 根阳线最高价触及压力带，且最后一次触及后的 n 根 K 线内突破压力带的窗口。</div>
        </div>
      </div>
    ...
    <section v-if="strategyGroups.length" class="panel strategy-panel">
      <div class="panel-heading">
        <div>
          ...
          <div class="panel-subtitle">按止盈区间分组展示；手续费按买卖合计扣减。点击某个止盈组或某笔交易，都会联动到对应 K 线压力带。</div>
        </div>
      </div>
      <div class="strategy-overview">
        <div class="strategy-overview-main">
          ...
          <span>手续费 {{ formatPercent(activeStrategyGroup?.feeRate || 0) }} · 交易 {{ activeStrategySummary?.tradeCount || 0 }} 笔</span>
        </div>
        <div class="strategy-overview-meta">
          <span>区间 {{ strategyForm.takeProfitRateStartPercent }}%-{{ strategyForm.takeProfitRateEndPercent }}%</span>
      ...
      <div class="strategy-group-grid">
        ...
        >
          <div class="strategy-group-head">
            ...
            <span :class="profitClass(group.summary.totalProfitUsd)">{{ formatUsd(group.summary.totalProfitUsd) }}</span>
          </div>
          <div class="strategy-group-stats">
            <span>胜率 {{ formatPercent(group.summary.winRate) }}</span>
            <span>平均 {{ formatPercent(group.summary.averageProfitRate) }}</span>
            <span>回撤 {{ formatDrawdownUsd(group.summary.maxDrawdownUsd) }}</span>
          </div>
        </button>
      </div>
      <div class="strategy-metrics-grid compact">
        <div class="strategy-metric-card"><span>交易数</span><strong>{{ activeStrategySummary?.tradeCount || 0 }}</strong></div>
        <div class="strategy-metric-card"><span>胜率</span><strong>{{ formatPercent(activeStrategySummary?.winRate || 0) }}</strong></div>
        <div class="strategy-metric-card"><span>净收益</span><strong :class="profitClass(activeStrategySummary?.totalProfitUsd || 0)">{{ formatUsd(activeStrategySummary?.totalProfitUsd || 0) }}</strong></div>
        <div class="strategy-metric-card"><span>平均净收益率</span><strong :class="profitClass(activeStrategySummary?.averageProfitRate || 0)">{{ formatPercent(activeStrategySummary?.averageProfitRate || 0) }}</strong></div>
        <div class="strategy-metric-card"><span>最大回撤</span><strong class="loss">{{ formatDrawdownUsd(activeStrategySummary?.maxDrawdownUsd || 0) }}</strong></div>
        <div class="strategy-metric-card"><span>总投入</span><strong>{{ formatUsd(activeStrategySummary?.committedCapitalUsd || 0) }}</strong></div>
      </div>
        ...
        >
          <div class="strategy-result-head">
            <div class="strategy-result-title">
              <strong>W{{ trade.windowIndex }} · {{ trade.levelType === 'support' ? '支撑' : '压力' }} {{ formatMarketCap(trade.levelMarketCap) }}</strong>
              <span class="strategy-result-hold">持有 {{ trade.holdingBars }} 根</span>
            </div>
            <span class="strategy-result-rate" :class="profitClass(trade.profitRate)">{{ formatPercent(trade.profitRate) }}</span>
          </div>
          <div class="strategy-result-grid">
            <div class="strategy-result-cell">
              <span class="strategy-result-label">买入</span>
              <strong>{{ formatShortTime(trade.buyPoint.time) }} @ {{ formatMarketCap(trade.buyPoint.marketCap) }}</strong>
            </div>
            <div class="strategy-result-cell">
              <span class="strategy-result-label">卖出</span>
              <strong>{{ formatShortTime(trade.sellPoint.time) }} @ {{ formatMarketCap(trade.sellPoint.marketCap) }}</strong>
            </div>
            <div class="strategy-result-cell">
              <span class="strategy-result-label">净收益</span>
              <strong :class="profitClass(trade.profitUsd)">{{ formatUsd(trade.profitUsd) }}</strong>
            </div>
            <div class="strategy-result-cell">
              <span class="strategy-result-label">毛收益 / 手续费成本</span>
              <strong>{{ formatUsd(trade.grossProfitUsd) }} / {{ formatCostUsd(trade.feeUsd) }}</strong>
            </div>
            <div class="strategy-result-cell">
              <span class="strategy-result-label">净收益率</span>
              <strong :class="profitClass(trade.profitRate)">{{ formatPercent(trade.profitRate) }}</strong>
            </div>
            <div class="strategy-result-cell">
          ...
          <div class="strategy-result-meta">
            <span>买卖点按突破市值回测，净收益已扣除总手续费 {{ formatPercent(trade.feeRate) }}</span>
          </div>
        </button>
      </div>
    </section>
    ...
    <section ref="chartPanelRef" class="panel chart-panel">
      <div class="panel-heading">
        <el-select v-if="levelOptions.length" v-model="selectedLevelKey" class="level-picker" placeholder="选择要解释的市值位">
          <el-option v-for="option in levelOptions" :key="option.key" :label="option.label" :value="option.key" />
        </el-select>
      </div>
      <KlineTradeChart
    ...
    <section v-if="sortedLevels.length" class="panel">
      <div class="panel-heading">
        <div>
          ...
          <div class="panel-subtitle">市值带来自滑动窗口内的局部高低点聚类；选中后可在 K 线上看到参与计算的点位。</div>
        </div>
      </div>
      <div class="level-detail-grid">
        <article
          ...
          :class="[level.type, { active: levelKey(level) === selectedLevelKey }]"
        ...
        >
          ...
          <div class="level-band">{{ formatMarketCap(level.lowerMarketCap) }} - {{ formatMarketCap(level.upperMarketCap) }}</div>
          <div class="level-explain">{{ level.calculation?.typeReason }}</div>
          <dl class="level-facts">
            <div><dt>Pivot</dt><dd>{{ level.calculation?.pivotCount || 0 }} 个</dd></div>
            <div><dt>触碰</dt><dd>{{ level.touches || 0 }} 次</dd></div>
            <div><dt>成交量</dt><dd>{{ formatCompact(level.volume) }}</dd></div>
            <div><dt>强度</dt><dd>{{ formatFixed(level.score) }}</dd></div>
            <div><dt>低点票</dt><dd>{{ level.calculation?.supportVotes || 0 }}</dd></div>
            <div><dt>高点票</dt><dd>{{ level.calculation?.resistanceVotes || 0 }}</dd></div>
          </dl>
          <div class="score-formula">
            <span>强度 = 触碰 {{ formatFixed(level.calculation?.scoreParts?.touch) }}</span>
            <span>+ 成交量 {{ formatFixed(level.calculation?.scoreParts?.volume) }}</span>
            <span>+ 最近性 {{ formatFixed(level.calculation?.scoreParts?.recency) }}</span>
            <span>+ 距离 {{ formatFixed(level.calculation?.scoreParts?.distance) }}</span>
          </div>
          <div class="level-explain">{{ level.calculation?.statusReason }}</div>
          <div v-if="level.breakout?.consolidation" class="breakout-card">
            <strong>平台整理区</strong>
            <span>整理区：{{ formatShortTime(level.breakout.consolidation.startTime) }} - {{ formatShortTime(level.breakout.consolidation.endTime) }}</span>
            <span>区间：{{ formatMarketCap(level.breakout.consolidation.lowPrice) }} - {{ formatMarketCap(level.breakout.consolidation.highPrice) }}</span>
            <span>包含 {{ level.breakout.consolidation.barCount }} 根 K 线，期间 {{ form.minTouches }} 根阳线最高价触及压力带</span>
            <span v-if="level.breakout.breakoutPoint">后 {{ form.minTouches }} 根内突破点：{{ formatShortTime(level.breakout.breakoutPoint.time) }} @ {{ formatMarketCap(level.breakout.breakoutPoint.marketCap) }}</span>
          </div>
...
const WINDOW_COLORS = ['#60a5fa', '#f59e0b', '#34d399', '#f472b6', '#a78bfa', '#f87171', '#22d3ee', '#facc15']
...
const tokenAddressOptions = computed(() => PRESET_TOKEN_ADDRESSES.map((item, index) => ({
  value: item,
  label: `#${index + 1} · ${shortenAddress(item)}`
})))
const barCount = computed(() => store.result?.klines?.length || 0)
const filteredWindows = computed(() => dedupeScenarioWindows(store.result?.windows || []))
const windowCount = computed(() => filteredWindows.value.length)
const windowOptions = computed(() => filteredWindows.value.map((window, index) => ({
...
const selectedWindow = computed(() => filteredWindows.value.find((window) => windowKey(window) === selectedWindowKey.value) || filteredWindows.value[0] || null)
const selectedWindowColor = computed(() => windowOptions.value.find((option) => option.key === selectedWindowKey.value)?.color || WINDOW_COLORS[0])
const selectedWindowLevels = computed(() => selectedWindow.value?.levels || [])
const visibleWindowLevels = computed(() => filterLevelsByView(selectedWindowLevels.value, levelView.value))
const supportCount = computed(() => selectedWindowLevels.value.filter(isSupportLevel).length)
const resistanceCount = computed(() => selectedWindowLevels.value.filter(isResistanceLevel).length)
...
const selectedStrategyMethod = computed(() => strategyMethodOptions.value.find((item) => item.code === strategyForm.methodCode) || null)
const strategyGroups = computed(() => store.strategyBacktestResult?.groups || [])
const activeStrategyGroup = computed(() => strategyGroups.value.find((group) => groupKey(group) === activeStrategyGroupKey.value) || strategyGroups.value[0] || null)
const activeStrategySummary = computed(() => activeStrategyGroup.value?.summary || null)
const activeStrategyTrades = computed(() => activeStrategyGroup.value?.trades || [])
const levelOptions = computed(() => sortedLevels.value.map((level) => ({
  key: levelKey(level),
  label: `${levelDisplayName(level)} ${formatMarketCap(level.marketCap)} · 强度 ${formatFixed(level.score)}`
})))
const selectedLevel = computed(() => sortedLevels.value.find((level) => levelKey(level) === selectedLevelKey.value) || sortedLevels.value[0] || null)
...
const rangeCardTitle = computed(() => (loadedRange.value?.source === '选中区域' ? '选中区域' : '固定近 5 天'))
...
async function loadRangeLevels(range, sourceLabel) {
  ...
  const initialLevel = sortLevels(filterLevelsByView(firstWindow?.levels || [], levelView.value))[0]
  ...
  ElMessage.success(`已加载 ${result.klines.length} 根 Birdeye K线，筛出 ${dedupeScenarioWindows(result.windows || []).length} 个压力突破场景窗口`)
}
...
async function runStrategyForLoadedRange() {
  ...
  ElMessage.success(`回测完成：共 ${result.groups?.length || 0} 个止盈组，最佳净收益 ${formatUsd(result.summary?.totalProfitUsd || 0)}`)
}

function handleChartRangeSelected(range) {
  selectedChartRange.value = range
  ElMessage.success(`已选中 ${range.klineCount} 根 K 线，可点击“计算选中区域压力位”`)
}
...
function selectWindow(key) {
  ...
  const targetWindow = filteredWindows.value.find((window) => windowKey(window) === key)
  const firstLevel = sortLevels(filterLevelsByView(targetWindow?.levels || [], levelView.value))[0]
...
function focusTrade(trade) {
  const window = filteredWindows.value.find((item) => item.windowIndex === trade.windowIndex)
  if (window) {
    selectedWindowKey.value = windowKey(window)
  }
  const level = (window?.levels || []).find((item) =>
...
function pickBestStrategyGroup(groups) {
  return [...groups].sort((left, right) => Number(right.summary?.totalProfitUsd || 0) - Number(left.summary?.totalProfitUsd || 0))[0] || null
}

function formatMarketCap(value) {
  const number = Number(value)
  if (!Number.isFinite(number)) return '-'
  if (Math.abs(number) >= 1_000_000) return `${(number / 1_000_000).toFixed(2).replace(/\.00$/, '').replace(/(\.\d)0$/, '$1')}m`
  if (Math.abs(number) >= 1_000) return `${(number / 1_000).toFixed(2).replace(/\.00$/, '').replace(/(\.\d)0$/, '$1')}k`
...
function formatCompact(value) {
  const number = Number(value)
  return Number.isFinite(number) ? Intl.NumberFormat('zh-CN', { notation: 'compact', maximumFractionDigits: 2 }).format(number) : '-'
}
...
function isBreakoutResistanceLevel(level) {
  return isResistanceLevel(level) && level.breakout?.consolidation && level.breakout?.breakoutPoint
}

function isSupportLevel(level) {
  return Number(level?.calculation?.supportVotes || 0) > 0 || level?.type === 'support'
}

function isResistanceLevel(level) {
  return Number(level?.calculation?.resistanceVotes || 0) > 0 || level?.type === 'resistance'
}
...
onMounted(async () => {
  await store.loadStrategyMethods()
})
</script>
