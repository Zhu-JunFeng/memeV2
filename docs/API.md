...
- 回测会把整段历史按时间顺序逐根 K 线回放；每新增一根 K 线，都复用实时买点判定检查“这一根是否是首个有效突破”，不再使用静态窗口结果直接反推买点。
- 在突破压力带的那根 K 线按突破阈值对应市值买入。
- 单个 token 同一时刻最多只保留 `1` 笔持仓；若前一笔尚未卖出，则后续买点信号全部跳过。
- 如果买入后任意一根后续 1 分钟 K 线正式收盘，且收盘市值低于压力带下沿，则按该根实际收盘市值止损卖出。
...
- 如果最高点达到 `takeProfitRate`，则按止盈价卖出；默认止盈比例为 `8%`。
...
- 先用历史窗口识别出已经形成的试压结构
- 再用当前实时 K 线判断是否真正突破
- 某组 `minTouches` 试压一旦形成，只认这组试压之后的第一根有效突破 K 线为买点；后面新增的试压不会把这次买点往后改写
- 一旦满足突破条件，就立即返回信号
...
- 候选池未持仓项目按配置周期调用 GMGN 最近 1m K 线；已买入项目使用独立的 `0.5s` 轮询持续检查盘中止盈和动态止损，同时在每根后续 1m K 线收盘后检查收盘跌破压力带下沿规则。系统保留上游真实 `volume`，并按价格乘 Solana RPC `getTokenSupply` 换算本地 1m 市值 K 线；重启时从 `system_kline_cache` 预加载后继续增量维护。
- XXYY/GMGN 项目源返回的项目只有在当前市值严格大于 `15_000` 时才允许首次进入候选池；市值缺失、等于或低于 `15_000` 的项目不入池。
- 买入信号成交后，候选持仓会从交易持仓表读取 Jupiter 实际成交均价，并乘 token supply 得到实际买入市值；后续 `8%` 止盈、`7%/4%` 动态止损和通知盈亏均以实际成交市值为成本基准。
- 已入池的未买入候选在最新市值低于 `15_000` 时从监控池移除；`signal.min_market_cap` 只允许配置更高阈值，不能把移除阈值降到 `15_000` 以下；已买入候选继续监控卖点
...
- `bought` 候选在每轮卖出判断前都会按买入信号查询未平仓持仓；数据库暂时缺失时再确认 Redis 运行时持仓。两处都不存在即视为已经平仓，结束旧 `bought` 状态，不再发布会被交易模块 `skipped` 的重复卖出信号。
- 买入信号发布前会先保存 `bought` 待确认状态和发布时间；信号持续 10 秒未落库且 Redis 没有实际持仓时自动释放发射锁并恢复监控。历史上没有发布时间的缺失信号会立即恢复，避免永久卡在 `bought`。
- 卖出成交删除 Redis 持仓失败时会进入异步重试；服务启动恢复持仓时会用数据库中的同一 position 状态识别并删除已关闭的 Redis 残留，避免已平仓持仓在重启后重新进入运行时。
...
- 交易信号、订单或成交异步落库失败时，该任务延迟重试并让出消费队列，单个永久失败任务不会阻塞后续交易记录；依赖记录未创建时继续重试，已成交订单不会被延迟到达的状态更新降级。
- 服务启动时会把 Redis 中仍为 `open` 的实时持仓合并回交易运行时。候选状态查不到数据库买入信号、但 Redis 确认存在未平仓仓位时，使用 Redis 实际成交价格和时间恢复卖出判断，不会重新发起买入。
