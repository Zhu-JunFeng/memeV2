...
- 如果买入后任意后续 1 分钟 K 线盘中最低市值触及买入价下方 `5%`，则按固定止损价卖出。
- 不再使用压力带下沿作为止损线。
...
- 候选池未持仓项目按配置周期调用 GMGN 最近 1m K 线；已买入项目使用独立的 `0.5s` 轮询持续检查固定 `5%` 止损、盘中止盈和动态止损。系统保留上游真实 `volume`，并按价格乘 Solana RPC `getTokenSupply` 换算本地 1m 市值 K 线；重启时从 `system_kline_cache` 预加载后继续增量维护。
- XXYY/GMGN 项目源返回的项目只有在当前市值严格大于 `15_000` 时才允许首次进入候选池；市值缺失、等于或低于 `15_000` 的项目不入池。
- 买入信号成交后，候选持仓会从交易持仓表读取 Jupiter 实际成交均价，并乘 token supply 得到实际买入市值；后续固定 `5%` 止损、`8%` 止盈、`7%/4%` 动态止损和通知盈亏均以实际成交市值为成本基准。
